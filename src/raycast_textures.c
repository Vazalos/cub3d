...
void	wall_texture(t_data *data, int x)
{
	...
	if (data->cast.side == 1)
		wall_x = data->cast.pov.x + data->cast.perpend_wall_dist
			* data->cast.ray_dir.x;
	else
		wall_x = data->cast.pov.y + data->cast.perpend_wall_dist
			* data->cast.ray_dir.y;
	wall_x -= floor(wall_x);
	texture_x = (int)(wall_x * (double)TEX_SIZE);
	if (data->cast.side == 1 && data->cast.ray_dir.y > 0)
		texture_x = TEX_SIZE - texture_x - 1;
	if (data->cast.side == 0 && data->cast.ray_dir.x > 0)
		texture_x = TEX_SIZE - texture_x - 1;

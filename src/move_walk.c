/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   move_walk.c                                        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: david-fe <david-fe@student.42.com>         +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/17 17:05:10 by david-fe          #+#    #+#             */
/*   Updated: 2025/10/23 17:16:49 by david-fe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/cub3d.h"

void	walk(t_data *data)
{
	double	player_radius;

	player_radius = 0.2;
	if (data->move.front == 1)
		walk_front(data, player_radius);
	if (data->move.back == 1)
		walk_back(data, player_radius);
	if (data->move.right == 1)
		walk_right(data, player_radius);
	if (data->move.left == 1)
		walk_left(data, player_radius);
}

void	walk_front(t_data *data, double rad)
{
	double	new_y;
	double	new_x;

	new_y = data->cast.pov.y + (data->cast.dir.y * data->move.speed);
	new_x = data->cast.pov.x + (data->cast.dir.x * data->move.speed);
	if (data->map->map[(int)new_y][(int)data->cast.pov.x] == '0'
		&& has_collision_y(data, new_y, rad) == 0)
		data->cast.pov.y = new_y;
	if (data->map->map[(int)data->cast.pov.y][(int)new_x] == '0'
		&& has_collision_x(data, new_x, rad) == 0)
		data->cast.pov.x = new_x;
}

void	walk_back(t_data *data, double rad)
{
	double	new_y;
	double	new_x;

	new_y = data->cast.pov.y - (data->cast.dir.y * data->move.speed);
	new_x = data->cast.pov.x - (data->cast.dir.x * data->move.speed);
	if (data->map->map[(int)new_y][(int)data->cast.pov.x] == '0'
		&& has_collision_y(data, new_y, rad) == 0)
		data->cast.pov.y = new_y;
	if (data->map->map[(int)data->cast.pov.y][(int)new_x] == '0'
		&& has_collision_x(data, new_x, rad) == 0)
		data->cast.pov.x = new_x;
}

void	walk_left(t_data *data, double rad)
{
	double	new_y;
	double	new_x;

	new_y = data->cast.pov.y - (data->cast.dir.x * data->move.speed);
	new_x = data->cast.pov.x + (data->cast.dir.y * data->move.speed);
	if (data->map->map[(int)new_y][(int)data->cast.pov.x] == '0'
		&& has_collision_y(data, new_y, rad) == 0)
		data->cast.pov.y = new_y;
	if (data->map->map[(int)data->cast.pov.y][(int)new_x] == '0'
		&& has_collision_x(data, new_x, rad) == 0)
		data->cast.pov.x = new_x;
}

void	walk_right(t_data *data, double rad)
{
	double	new_y;
	double	new_x;

	new_y = data->cast.pov.y + (data->cast.dir.x * data->move.speed);
	new_x = data->cast.pov.x - (data->cast.dir.y * data->move.speed);
	if (data->map->map[(int)new_y][(int)data->cast.pov.x] == '0'
		&& has_collision_y(data, new_y, rad) == 0)
		data->cast.pov.y = new_y;
	if (data->map->map[(int)data->cast.pov.y][(int)new_x] == '0'
		&& has_collision_x(data, new_x, rad) == 0)
		data->cast.pov.x = new_x;
}
